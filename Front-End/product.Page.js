...
let PRODUCTS_API_URL = "http://52.90.2.111:3000/api/store/products";
...
async function checkIdentity() {
    try {
        ...
        const response = await axios.get("http://52.90.2.111:3000/api/users/me", {
...
async function AddToCart(perchaseInfo) {
    try {
        ...
        const response = await axios.post("http://52.90.2.111:3000/api/cards/add-to-cart", perchaseInfo, {
            headers: {
        ...
        if (user.card != response.data.card._id) {
            ...
            await axios.put(`http://52.90.2.111:3000/api/users/${user._id}`, { card: user.card }, {
                headers: {
                    'Content-Type': 'application/json',
                }
            });
