...
let PRODUCTS_API_URL = "/api/store/products";
...
async function AddToCart(perchaseInfo) {
    try {
        ...
        const response = await axios.post("/api/cards/add-to-cart", perchaseInfo, {
            headers: {
        ...
        if (user.card != response.data.card._id) {
            ...
            await axios.put(`/api/users/${user._id}`, { card: user.card }, {
                headers: {
                    'Content-Type': 'application/json',
                }
            });
