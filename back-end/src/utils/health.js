...
const healthCheck = async (req, res) => {
    try {
        ...
        const overallStatus = (dbHealth.status === 'healthy' && mailerHealth.status === 'healthy') ? 'healthy' : 'degraded';
        ...
        res.status(500).json({ status: 'error', message: 'Health check failed', error: error.message, timestamp: new Date().toISOString() });
    }
};
